...
//        self.addSubview(rightLineView)
    ...
    func config(_ config: RootTabBarConfig) {
        ...
        self.backgroundImage = config.tabBarBackgroundImg ?? UIColor.colorToImage(color: config.tabBarBackgroundColor!,size: CGSize(width: screenWidth, height: self.bounds.height))
        configModel = config
    }
        ...
        for barButton in self.subviews {
            if barButton.isKind(of: NSClassFromString("UITabBarButton")!){
                if configModel.tabBarStyle == .center && addButton.currentImage != nil {
                    ...
                    barButton.frame = CGRect(x: buttonX * CGFloat(index), y: 0, width: buttonX, height: 49)
                }
                index += 1
            }
        }
    ...
    func animationTap(_ tapView: UIView?) {
        ...
        var imgView : UIView?
        for tmpImgView in tapView!.subviews {
            if tmpImgView.isKind(of: NSClassFromString("UITabBarSwappableImageView")!) {
                imgView = tmpImgView as! UIImageView
            }
        }
        if imgView == nil {
            imgView = tapView
        ...
        if configModel.animation == .scaleDown {
            ...
            imgView!.layer.add(impliesAnimation, forKey: nil)
            ...
            imgView!.layer.add(scaleAnim, forKey: nil)
        } else {
